...
$pageTitle = __('nav.sign_up') . ' - TravelQuest';
$showNavbar = false;
$showFooter = false;
$error = $error ?? '';
ob_start();
...
<div class="min-h-screen flex items-center justify-center py-12 px-4 relative overflow-hidden" style="background-image: url('https://images.unsplash.com/photo-1506905925346-21bda4d32df4?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2070&q=80'); background-size: cover; background-position: center; background-repeat: no-repeat;">
    ...
    <div class="max-w-md w-full relative z-10">
        <div class="text-center mb-8">
            ...
            <h2 class="text-2xl font-bold text-white mb-2 drop-shadow-lg"><?= __('auth.create_account') ?></h2>
            <p class="text-white/90 drop-shadow-md"><?= __('auth.create_account_subtitle') ?></p>
        </div>
        ...
        <div class="bg-white/10 backdrop-blur-md rounded-xl shadow-2xl p-8 border border-white/20">
            ...
            <form method="POST" action="<?= url('/register') ?>" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="full_name" required placeholder="<?= __('auth.full_name_placeholder') ?>" class="w-full pl-10 pr-4 py-3 bg-white/20 backdrop-blur-sm border border-white/30 rounded-lg text-white placeholder-white/60 focus:ring-2 focus:ring-white/50 focus:border-white/50 focus:bg-white/30 transition-all">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="password" name="password" required placeholder="<?= __('auth.password_create') ?>" class="w-full pl-10 pr-4 py-3 bg-white/20 backdrop-blur-sm border border-white/30 rounded-lg text-white placeholder-white/60 focus:ring-2 focus:ring-white/50 focus:border-white/50 focus:bg-white/30 transition-all">
                    </div>
                </div>

                <div>
                    <label class="block text-sm font-medium text-white mb-2 drop-shadow-md"><?= __('auth.password_confirm') ?></label>
                    <div class="relative">
                        <i class="fas fa-lock absolute left-3 top-1/2 transform -translate-y-1/2 text-white/70"></i>
                        <input type="password" name="confirm_password" required placeholder="<?= __('auth.password_confirm_placeholder') ?>" class="w-full pl-10 pr-4 py-3 bg-white/20 backdrop-blur-sm border border-white/30 rounded-lg text-white placeholder-white/60 focus:ring-2 focus:ring-white/50 focus:border-white/50 focus:bg-white/30 transition-all">
                    </div>
                </div>

                <div class="flex items-start">
                    <input type="checkbox" required class="w-4 h-4 text-blue-600 border-white/30 rounded focus:ring-white/50 mt-1 bg-white/20">
                    <label class="ml-2 text-sm text-white drop-shadow-md">
                        <?= __('auth.terms_agree') ?> <a href="#" class="text-white hover:text-white/80 underline transition-colors"><?= __('auth.terms') ?></a> <?= __('auth.and') ?> <a href="#" class="text-white hover:text-white/80 underline transition-colors"><?= __('auth.privacy') ?></a>
                    </label>
                </div>

                <button type="submit" class="w-full bg-blue-600/90 backdrop-blur-sm text-white py-3 rounded-lg hover:bg-blue-700/90 transition-all font-semibold shadow-lg border border-white/20"><?= __('auth.create_account_btn') ?></button>
            </form>
            ...
            <p class="mt-6 text-center text-sm text-white/90 drop-shadow-md">
                <?= __('auth.already_have_account') ?> <a href="<?= url('/login') ?>" class="text-white font-medium hover:text-white/80 transition-colors underline"><?= __('auth.sign_in') ?></a>
            </p>
        </div>
    </div>
</div>
