...
/**
 * Update booking VNPay transaction reference
 */
function update_booking_vnpay_ref($bookingId, $vnp_TxnRef) {
    $db = get_db();
    $stmt = $db->prepare("
        UPDATE bookings 
...
/**
 * Update booking payment status (for VNPay)
 */
function update_booking_payment_status($bookingId, $status, $vnp_TxnRef = null, $vnp_TransactionNo = null, $vnp_ResponseCode = null, $vnp_TransactionStatus = null, $vnp_SecureHash = null) {
    $db = get_db();
    ...
    if ($vnp_TxnRef !== null) {
        $fields[] = 'vnp_TxnRef = :vnp_TxnRef';
        $params[':vnp_TxnRef'] = $vnp_TxnRef;
    }
    
    if ($vnp_TransactionNo !== null) {
        $fields[] = 'vnp_TransactionNo = :vnp_TransactionNo';
        $params[':vnp_TransactionNo'] = $vnp_TransactionNo;
    }
    
    if ($vnp_ResponseCode !== null) {
        $fields[] = 'vnp_ResponseCode = :vnp_ResponseCode';
        $params[':vnp_ResponseCode'] = $vnp_ResponseCode;
    }
    
    if ($vnp_TransactionStatus !== null) {
        $fields[] = 'vnp_TransactionStatus = :vnp_TransactionStatus';
        $params[':vnp_TransactionStatus'] = $vnp_TransactionStatus;
    }
    
    if ($vnp_SecureHash !== null) {
        $fields[] = 'vnp_SecureHash = :vnp_SecureHash';
        $params[':vnp_SecureHash'] = $vnp_SecureHash;
    }
    
    if ($status === 'paid') {
        $fields[] = "payment_method = 'VNPay'";
        $fields[] = "payment_gateway = 'vnpay'";
    }
